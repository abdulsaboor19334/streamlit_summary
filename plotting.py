...
def top_10_by_sale(df):
    ...
    return category_wise_sale.sort_values("total_sale",ascending=False).head()

def top_10_by_quantity(df):
    category_wise_sale = df.groupby("category_id",as_index=False).agg(
        total_count = pd.NamedAgg(column="shop_total",aggfunc="count"),
        total_sale = pd.NamedAgg(column="shop_total",aggfunc="sum"),
        total_quantity = pd.NamedAgg(column="quantity",aggfunc="sum"),
    ...
    return category_wise_sale.sort_values("total_quantity",ascending=False).head()
